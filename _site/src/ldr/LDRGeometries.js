...
LDR.mergeGeometries = function (geometries) {
  ...
  while (geometries.length > 1) {
    // Repeat rounds until only 1 left:
...
LDR.LDRGeometry.prototype.buildConditionalLineGeometries = function (
  vertexAttribute
) {
  for (let c in this.conditionalLines) {
    ...
    g.setAttribute("position", new Float32BufferAttribute(p1s, 3));
    g.setAttribute("p2", new Float32BufferAttribute(p2s, 3));
    g.setAttribute("p3", new Float32BufferAttribute(p3s, 3));
    g.setAttribute("p4", new Float32BufferAttribute(p4s, 3));
...
LDR.LDRGeometry.prototype.buildBothLineGeometries = function () {
  ...
  let vertexAttribute = new Float32BufferAttribute(lineVertices, 3);
...
LDR.LDRGeometry.prototype.buildGeometries = function () {
  ...
  allTriangleColors.forEach((c) => {
    ...
    let hv = function (idx) {
      ...
      if (v.c !== colorIdx) {
        // Not already seen for this color.
        v.c = colorIdx;
        v.idx = tvIdx++;
    ...
    handlePrimitives(self.triangles, (t) =>
      triangleIndices.push(hv(t.p1), hv(t.p2), hv(t.p3))
    );
    ...
    let triangleVertexAttribute = new Float32BufferAttribute(
      triangleVertices,
      3
    );
    let triangleGeometry = this.buildGeometry(
      triangleIndices,
...
LDR.LDRGeometry.prototype.buildPhysicalGeometries = function (
  ...
  for (let c in this.lines) {
    if (this.lines.hasOwnProperty(c)) {
      let lines = this.lines[c];
      lines.forEach(
        (line) => (vertices[line.p1].hard = vertices[line.p2].hard = true)
      );
  ...
  function updateTriangleIndices(t) {
    ...
    let h1 = vertices[p1].hard,
      h2 = vertices[p2].hard,
      h3 = vertices[p3].hard;
    let soft12 = softEdges[key(p1, p2)],
      soft23 = softEdges[key(p2, p3)],
      soft31 = softEdges[key(p3, p1)];
  ...
  function updateQuadIndices(t) {
    ...
    let soft12 = softEdges[key(p1, p2)],
      soft23 = softEdges[key(p2, p3)],
      soft34 = softEdges[key(p3, p4)],
      soft41 = softEdges[key(p4, p1)];
  ...
  let triangleVertexAttribute = new Float32BufferAttribute(triangleVertices, 3);
  ...
  allTriangleColors.forEach((c) => {
    ...
    let triangles = self.triangles.hasOwnProperty(c)
      ? self.triangles[c].filter((p) => !p.t)
      : [];
    let triangles2 = self.triangles2.hasOwnProperty(c)
      ? self.triangles2[c].filter((p) => !p.t)
      : [];
    let quads = self.quads.hasOwnProperty(c)
      ? self.quads[c].filter((p) => !p.t)
      : [];
    let quads2 = self.quads2.hasOwnProperty(c)
      ? self.quads2[c].filter((p) => !p.t)
      : [];
    ...
    let [UVU, UVV] = [
      ...
      [0, 0],
      [0.5, 0],
    ][Math.floor(Math.random() * 3)]; // Which of the 3 cells to use for the UV mapping (not 1,1 with the LEGO logo)

    function setUVs(indices) {
      const len = indices.length;
      let maxDiff = (xs) =>
        xs
          .map((x, idx, a) => Math.abs(x - a[idx === 0 ? len - 1 : idx - 1]))
          .reduce((a, b) => (a > b ? a : b), 0);
      let vs = indices.map((i) => vertices[i]);
      let ns = indices
        .map((i) => 3 * i)
        .map(
          (idx) => new Vector3(normals[idx], normals[1 + idx], normals[2 + idx])
        );
      let N = ns.reduce(
      ...
      function setUV(fu, fv, force) {
        ...
        if (!force) {
          ...
          let turn = (uv) =>
            (prev.u - prevprev.u) * (uv.v - prevprev.v) -
            (prev.v - prevprev.v) * (uv.u - prevprev.u);
          for (let i = 0; i < ret.length; i++) {
            let uv = ret[i];
            if (
              (Math.abs(prev.u - uv.u) < LDR.EPS &&
                Math.abs(prev.v - uv.v) < LDR.EPS) ||
              (Math.abs(prevprev.u - uv.u) < LDR.EPS &&
                Math.abs(prevprev.v - uv.v) < LDR.EPS) ||
      ...
      if (atLeast3EqualNormals()) {
        // Just project onto the plane where the normals point the most:
        // First check if this is a simple rectilinear face:
        let DX, DY, DZ;
        if (vs.some((v) => v.o === true)) {
          // Logo position: Overwrite setUV():
      ...
      if (
        (NX < 1e-7 &&
          setUV((v, i) => dxyz(v) + toCircle(ns[i].y, ns[i].z), dx, false)) ||
        (NY < 1e-7 &&
          setUV((v, i) => dxyz(v) + toCircle(ns[i].x, ns[i].z), dy, false)) ||
        (NZ < 1e-7 &&
          setUV((v, i) => dxyz(v) + toCircle(ns[i].x, ns[i].y), dz, false))
      ) {
        return;
      }
...
LDR.LDRGeometry.prototype.replaceWithDeep = function (g) {
  ...
  g.vertices.forEach((v) =>
    self.vertices.push({ x: v.x, y: v.y, z: v.z, o: v.o })
  );
  ...
  for (let c in g.conditionalLines) {
    ...
    let ps = [];
    g.conditionalLines[c].forEach((p) =>
  ...
  for (let c in g.triangles) {
    ...
    let ps = [];
    g.triangles[c].forEach((p) =>
      ps.push({ p1: p.p1, p2: p.p2, p3: p.p3, t: p.t })
    );
    this.triangles[c] = ps;
  }
  ...
  for (let c in g.quads) {
    ...
    let ps = [];
    g.quads[c].forEach((p) =>
      ps.push({ p1: p.p1, p2: p.p2, p3: p.p3, p4: p.p4, t: p.t })
    );
    this.quads[c] = ps;
  }
...
*/
LDR.LDRGeometry.prototype.fromPrimitives = function (
  lines,
  conditionalLines,
  triangles,
  quads
 ...
 */
LDR.LDRGeometry.prototype.sortAndBurnVertices = function (
  vertices,
  primitives
) {
...
LDR.LDRGeometry.prototype.fromLines = function (ps) {
  ...
  for (let i = 0; i < ps.length; i++) {
    ...
    vertices.push(
      { x: p.p1.x, y: p.p1.y, z: p.p1.z, c: p.c, idx: idx, p: 1 },
...
LDR.LDRGeometry.prototype.fromTriangles = function (cull, ps) {
  ...
  ps.forEach((p) => {
    ...
    vertices.push(
      { x: p.p1.x, y: p.p1.y, z: p.p1.z, c: p.c, idx: idx, p: 1, t: p.tmp },
...
LDR.LDRGeometry.prototype.fromPartDescription = function (loader, pd) {
  ...
  this.lines = t(this.lines, (p) => {
    return { p1: newIndices[p.p1], p2: newIndices[p.p2] };
  });
