...
LDR.STUDIO.handlePart = function (loader, pt) {
  ...
  if (
    step.subModels.length > 0 &&
    step.triangles.length > 0 &&
    step.lines.length === 0
  ) {
...
LDR.STUDIO.handleTriangleLine = function (pt, parts) {
  ...
  let p1 = new Vector3();
  {
    ...
    if (
      !isZero(u.z) &&
      !((isZero(n2.x) && isZero(n1.x)) || (isZero(n2.y) && isZero(n1.y)))
    ) {
      // Lock z=0 and find p1:
      /* n1.p1 = A1, n2.p1 = A2, so:
            ...
            */
      let x = isZero(n2.y)
        ? (A2 - (n2.y / n1.y) * A1) / (n2.x - n1.x * (n2.y / n1.y))
        : (A1 - (n1.y / n2.y) * A2) / (n1.x - n2.x * (n1.y / n2.y));
      let y = isZero(n2.x)
        ? (A2 - (n2.x / n1.x) * A1) / (n2.y - n1.y * (n2.x / n1.x))
        : (A1 - (n1.x / n2.x) * A2) / (n1.y - n2.y * (n1.x / n2.x));
      p1.set(x, y, 0);
    } else if (
      !isZero(u.y) &&
      !((isZero(n2.x) && isZero(n1.x)) || (isZero(n2.z) && isZero(n1.z)))
    ) {
      // Same for y, so substitude z for y:
      let x = isZero(n2.z)
        ? (A2 - (n2.z / n1.z) * A1) / (n2.x - n1.x * (n2.z / n1.z))
        : (A1 - (n1.z / n2.z) * A2) / (n1.x - n2.x * (n1.z / n2.z));
      let z = isZero(n2.x)
        ? (A2 - (n2.x / n1.x) * A1) / (n2.z - n1.z * (n2.x / n1.x))
        : (A1 - (n1.x / n2.x) * A2) / (n1.z - n2.z * (n1.x / n2.x));
      p1.set(x, 0, z);
    } else if (
      !isZero(u.x) &&
      !((isZero(n2.y) && isZero(n1.y)) || (isZero(n2.z) && isZero(n1.z)))
    ) {
      // Same for x:
      let y = isZero(n2.z)
        ? (A2 - (n2.z / n1.z) * A1) / (n2.y - n1.y * (n2.z / n1.z))
        : (A1 - (n1.z / n2.z) * A2) / (n1.y - n2.y * (n1.z / n2.z));
      let z = isZero(n2.y)
        ? (A2 - (n2.y / n1.y) * A1) / (n2.z - n1.z * (n2.y / n1.y))
        : (A1 - (n1.y / n2.y) * A2) / (n1.z - n2.z * (n1.y / n2.y));
      p1.set(0, y, z);
    } else {
      return; // Bail!
    }
