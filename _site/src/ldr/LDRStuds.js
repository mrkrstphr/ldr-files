...
import { LDRLoader, LDRPartDescription } from "./LDRLoader";
...
LDR.Studs.setStuds = function (loader, highContrast, logoType, onDone) {
  ...
  LDR.Studs.all.forEach((f) => {
    let [pt, id] = f(highContrast, logoType, force);
    id = id + ".dat";
    if (partTypes.hasOwnProperty(id)) {
      // Used, so replace and ensure sub models are present:
      partTypes[id] = pt;
      pt.ID = id;
  ...
  if (idb.length === 0) {
    ...
    let loader2 = new LDRLoader(onDone, loader.storage, options);
...
LDR.Studs.stud1 = function (highContrast, logoType, force, withoutBaseEdge) {
  let [pt, step] = LDR.Generator.pT(
    "Stud" + (withoutBaseEdge ? " without Base Edges" : "")
  );
  ...
  let logoSM = new LDRPartDescription(
    16,
    p4,
    r61,
    force + "4-4disc.dat",
...
LDR.Studs.studEl = function (highContrast, logoType, force) {
  ...
  step.addLine(24, new Vector3(-6, 0, 0), new Vector3(-6, -3, 0));

  step.addLine(24, new Vector3(0, 0, -6), new Vector3(0, -3, -6));
...
LDR.Studs.stud10 = function (highContrast, logoType, force) {
  ...
  // Lines:
  step.al([
    6, 0, 0, 5.6145, 0, 1.9397, 1.9387, 0, 5.6145, 0, 0, 6, 5.6145, -4, 1.9397,
    5.6145, 0, 1.9397, 6, -4, 0, 5.6145, -4, 1.9397, 5.6145, -4, 1.9397, 4.142,
    -4, 4.142, 4.142, -4, 4.142, 1.9397, -4, 5.6145, 1.9397, -4, 5.6145, 0, -4,
    6, 1.9397, -4, 5.6145, 1.9387, 0, 5.6145,
  ]);
  step.acl([
    4.142, -4, 4.142, 4.142, 0, 4.142, 1.9397, -4, 5.6145, 5.6145, -4, 1.9397,
    6, -4, 0, 6, 0, 0, 5.5434, -4, -2.2962, 5.6145, -4, 1.9397, 0, -4, 6, 0, 0,
    6, 1.9397, -4, 5.6145, -2.2962, -4, 5.5434,
  ]);
  ...
  step.at(
    [
      6, -4, 0, 5.6145, -4, 1.9397, 0, -4, 0, 5.6145, -4, 1.9397, 4.142, -4,
      4.142, 0, -4, 0, 4.142, -4, 4.142, 1.9397, -4, 5.6145, 0, -4, 0, 1.9397,
      -4, 5.6145, 0, -4, 6, 0, -4, 0,
    ],
    16
  );
  ...
  step.aq(
    [
      6, 0, 0, 5.6145, 0, 1.9397, 5.6145, -4, 1.9397, 6, -4, 0, 5.6145, 0,
      1.9397, 4.142, 0, 4.142, 4.142, -4, 4.142, 5.6145, -4, 1.9397, 4.142, 0,
      4.142, 1.9387, 0, 5.6145, 1.9397, -4, 5.6145, 4.142, -4, 4.142, 1.9387, 0,
      5.6145, 0, 0, 6, 0, -4, 6, 1.9397, -4, 5.6145,
    ],
    cc
  );
...
LDR.Studs.stud15 = function (highContrast, logoType, force) {
  ...
  step.asm(new Vector3(0.9694, -2, 5.8072), r0, force + "rect2p", cc);
  let r1 = new Matrix3();
  r1.set(0, -1.0502, -1.8379, 2, 0, 0, 0, -1.0502, 1.8379);
  step.asm(new Vector3(3.7766, -2, 3.7766), r1, force + "rect3", cc);
  let r2 = new Matrix3();
  r2.set(0, -7.7548, -0.1928, 2, 0, 0, 0, -1.542, 0.9694);
  step.asm(new Vector3(5.8072, -2, 0.9694), r2, force + "rect3", cc);
  step.asm(p4, r64, force + "3-4cyli", cc);
  ...
  step.acl([
    6, -4, 0, 6, 0, 0, 5.6145, -4, 1.9387, 5.5434, 0, -2.2962, 0, -4, 6, 0, 0,
    6, 1.9387, -4, 5.6145, -2.2962, 0, 5.5434,
  ]);
...
LDR.Studs.stud17 = function (highContrast, logoType, force, withoutBaseEdges) {
  let [pt, step] = LDR.Generator.pT(
    "Stud Open For Octagonal Parts" +
      (withoutBaseEdges ? " without Base Edges" : "")
  );
  let cc = highContrast ? 0 : 16;
  ...
  if (!withoutBaseEdges) {
    ...
    step.al([2.296, 0, 5.543, 2.78, 0, 5.22, 5.22, 0, 2.78, 5.543, 0, 2.296]);
  }

  step.al([
    2.78, -4, 5.22, 5.22, -4, 2.78, 2.296, -4, 5.543, 2.78, -4, 5.22, 5.22, -4,
    2.78, 5.543, -4, 2.296,
  ]);
  step.at([5.22, -4, 2.78, 2.78, -4, 5.22, 2.828, -4, 2.828], 16);
  step.aq(
    [
      2.78, 0, 5.22, 2.78, -4, 5.22, 5.22, -4, 2.78, 5.22, 0, 2.78, 2.296, 0,
      5.543, 2.296, -4, 5.543, 2.78, -4, 5.22, 2.78, 0, 5.22, 5.543, -4, 2.296,
      5.543, 0, 2.296, 5.22, 0, 2.78, 5.22, -4, 2.78,
    ],
    cc
  );
  step.aq(
    [
      2.296, -4, 5.543, 1.531, -4, 3.696, 2.828, -4, 2.828, 2.78, -4, 5.22,
      3.696, -4, 1.531, 5.543, -4, 2.296, 5.22, -4, 2.78, 2.828, -4, 2.828,
    ],
    16
  );
  step.acl([
    5.22, -4, 2.78, 5.22, 0, 2.78, 2.78, -4, 5.22, 5.543, -4, 2.296, 2.78, -4,
    5.22, 2.78, 0, 5.22, 5.22, -4, 2.78, 2.296, -4, 5.543, 2.296, -4, 5.543,
    2.296, 0, 5.543, 2.78, -4, 5.22, 0, -4, 6, 5.543, -4, 2.296, 5.543, 0,
    2.296, 5.22, -4, 2.78, 6, -4, 0,
  ]);
...
LDR.Studs.stud13 = function (highContrast, logoType, force) {
  ...
  step.al([
    6, 0, 0, 5.782, 0, 1.095, 5.782, 0, 1.095, 1.095, 0, 5.782, 1.095, 0, 5.782,
    0, 0, 6, 5.782, 0, 1.095, 5.782, -4, 1.095, 1.095, 0, 5.782, 1.095, -4,
    5.782, 6, -4, 0, 5.782, -4, 1.095, 5.782, -4, 1.095, 1.095, -4, 5.782,
    1.095, -4, 5.782, 0, -4, 6,
  ]);
  step.aq(
    [
      6, -4, 0, 6, 0, 0, 5.782, 0, 1.095, 5.782, -4, 1.095, 1.095, 0, 5.782,
      1.095, -4, 5.782, 5.782, -4, 1.095, 5.782, 0, 1.095, 1.095, -4, 5.782,
      1.095, 0, 5.782, 0, 0, 6, 0, -4, 6,
    ],
    cc
  );
  step.at(
    [
      6, -4, 0, 5.782, -4, 1.095, 0, -4, 0, 5.782, -4, 1.095, 1.095, -4, 5.782,
      0, -4, 0, 0, -4, 6, 0, -4, 0, 1.095, -4, 5.782,
    ],
    16
  );
  step.acl([
    6, 0, 0, 6, -4, 0, 5.782, 0, 1.095, 5.543, -4, -2.296, 0, 0, 6, 0, -4, 6,
    1.095, 0, 5.782, -2.296, -4, 5.543,
  ]);
...
LDR.Studs.stud6 = function (highContrast, logoType, force, withoutBaseEdges) {
  ...
  step.al([
    5.6145, -4, 1.9397, 5.6145, 0, 1.9397, 6, -4, 0, 5.6145, -4, 1.9397, 5.6145,
    -4, 1.9397, 4.142, -4, 4.142, 4.142, -4, 4.142, 1.9387, -4, 5.6145, 1.9387,
    -4, 5.6145, 0, -4, 6, 1.9387, -4, 5.6145, 1.9387, 0, 5.6145,
  ]);
  ...
  if (!withoutBaseEdges) {
    ...
    step.al([6, 0, 0, 5.6145, 0, 1.9397, 1.9387, 0, 5.6145, 0, 0, 6]);
  }

  step.aq(
    [
      6, -4, 0, 5.615, -4, 1.94, 3.695, -4, 1.531, 4, -4, 0, 5.615, -4, 1.94,
      4.142, -4, 4.142, 2.828, -4, 2.828, 3.695, -4, 1.531, 4.142, -4, 4.142,
      1.94, -4, 5.615, 1.531, -4, 3.695, 2.828, -4, 2.828, 1.94, -4, 5.615, 0,
      -4, 6, 0, -4, 4, 1.531, -4, 3.695,
    ],
    16
  );
  step.aq(
    [
      6, 0, 0, 5.6145, 0, 1.9397, 5.6145, -4, 1.9397, 6, -4, 0, 5.6145, 0,
      1.9397, 4.142, 0, 4.142, 4.142, -4, 4.142, 5.6145, -4, 1.9397, 4.142, 0,
      4.142, 1.9387, 0, 5.6145, 1.9387, -4, 5.6145, 4.142, -4, 4.142, 1.9387, 0,
      5.6145, 0, 0, 6, 0, -4, 6, 1.9387, -4, 5.6145,
    ],
    cc
  );
  step.acl([
    4.142, -4, 4.142, 4.142, 0, 4.142, 2.2962, -4, 5.5434, 5.5434, -4, 2.2962,
  ]);
