...
const OrbitControls = function (object, domElement) {
  ...
  var pan = (function () {
    ...
    return function pan(deltaX, deltaY) {
      ...
      if (scope.object.isPerspectiveCamera) {
        ...
        // we use only offsetHeight here so aspect ratio does not distort speed
        panLeft(
          (2 * deltaX * targetDistance) / element.offsetHeight,
          scope.object.matrix
        );
        panUp(
          (2 * deltaY * targetDistance) / element.offsetHeight,
          scope.object.matrix
        );
  ...
  function handleMouseMoveRotate(event) {
    ...
    rotateLeft((2 * Math.PI * rotateDelta.x) / element.offsetHeight); // yes, height

    rotateUp((2 * Math.PI * rotateDelta.y) / element.offsetHeight);
  ...
  this.panTo = function (x, y) {
    ...
    var divW = scope.object.zoom * element.offsetWidth;
    var divH = scope.object.zoom * element.offsetHeight;
    panUp(
      (y * (scope.object.top - scope.object.bottom)) / divH,
      scope.object.matrix
    );
    panLeft(
      (x * (scope.object.right - scope.object.left)) / divW,
      scope.object.matrix
    );
  ...
  function inTriggerBox(event) {
    if (!scope.noTriggerSize) return true;
    var w = scope.domElement.offsetWidth;
    var h = scope.domElement.offsetHeight;
    ...
    return (
      x > w * scope.noTriggerSize &&
      x < w - w * scope.noTriggerSize &&
      y > h * scope.noTriggerSize &&
      y < h - h * scope.noTriggerSize
    );
  }
