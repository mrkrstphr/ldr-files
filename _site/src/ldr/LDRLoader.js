 ...
 */
const LDRLoader = function (onLoad, storage, options) {
  let self = this;
  ...
  this.onFileLoadedd = this.options.onFileLoadedd || function () {};
 ...
 */
LDRLoader.prototype.loadMultiple = function (ids) {
  let self = this;
 ...
 */
LDRLoader.prototype.reportProgress = function (id) {
  this.onProgress(id);
 ...
 */
LDRLoader.textureLoader = new TextureLoader();
    ...
    switch (lineType) {
      case 0:
        ...
        function handleFileLine(originalFileName) {
          let fileName = originalFileName.replace('\\', '/'); // Normalize the name by bringing to lower case and replacing backslashes:
          localCull = true;
        ...
        if (is('FILE')) {
          ...
          if (parts.length >= 5) {
            step.rotation = new LDRStepRotation(
              parts[2],
              parts[3],
              parts[4],
          ...
          if (texmapPlacement) {
            ...
            if (!(parts.length === 3 && (parts[2] === 'FALLBACK' || parts[2] === 'END'))) {
              self.onWarning({
                message:
                  'Unexpected !TEXMAP line. Expected FALLBACK or END line. Found: "' + line + '".',
                line: i,
                subModel: part.ID,
              });
      ...
      case 1: // 1 <colour> x y z a b c d e f g h i <file>
        ...
        let position = new Vector3(parts[2], parts[3], parts[4]);
        ...
        let subModel = new LDRPartDescription(
          colorID,
          position,
          rotation,
          subModelID,
      ...
      case 2: // Line "2 <colour> x1 y1 z1 x2 y2 z2"
        p1 = new Vector3(parseFloat(parts[2]), parseFloat(parts[3]), parseFloat(parts[4]));
        p2 = new Vector3(parseFloat(parts[5]), parseFloat(parts[6]), parseFloat(parts[7]));
      ...
      case 3: // 3 <colour> x1 y1 z1 x2 y2 z2 x3 y3 z3 [u1 v1 u2 v2 u3 v3]
        p1 = new Vector3(parseFloat(parts[2]), parseFloat(parts[3]), parseFloat(parts[4]));
        p2 = new Vector3(parseFloat(parts[5]), parseFloat(parts[6]), parseFloat(parts[7]));
        p3 = new Vector3(parseFloat(parts[8]), parseFloat(parts[9]), parseFloat(parts[10]));
      ...
      case 4: // 4 <colour> x1 y1 z1 x2 y2 z2 x3 y3 z3 x4 y4 z4
        ...
        p2 = new Vector3(parseFloat(parts[5]), parseFloat(parts[6]), parseFloat(parts[7]));
        p3 = new Vector3(parseFloat(parts[8]), parseFloat(parts[9]), parseFloat(parts[10]));
        p4 = new Vector3(parseFloat(parts[11]), parseFloat(parts[12]), parseFloat(parts[13]));
...
LDRLoader.prototype.setReferencedFrom = function () {
  let self = this;
...
LDRLoader.prototype.toLDR = function () {
  let self = this;
...
LDRLoader.prototype.substituteReplacementParts = function () {
  let self = this;
...
LDRLoader.prototype.unpack = function (obj) {
  let self = this;
...
LDRLoader.prototype.pack = function () {
  let self = this;
...
*/
const LDRPartDescription = function (colorID, position, rotation, ID, cull, invertCCW, tmp = null) {
  ...
  this.original = undefined; // If this PD is a colored clone of an original PD.
  this.commentLines = [];
  if (tmp) {
    tmp.use();
  }
...
LDRPartDescription.prototype.cloneColored = function (colorID) {
  if (this.original) {
    console.dir(this);
  ...
  let ret = new LDRPartDescription(c, this.p, this.r, this.ID, this.cull, this.invertCCW, this.tmp);
...
LDRPartDescription.prototype.placedColor = function (pdColorID) {
  let c = this.c;
  if (c === 16) {
    c = pdColorID;
  } else if (c === 24) {
...
LDRPartDescription.prototype.toLDR = function (loader) {
...
LDRPartDescription.prototype.placeAt = function (pd) {
  ...
  let ret = new LDRPartDescription(c, p, r, this.ID, this.cull, invert, this.tmp);
  ret.commentLines.push(...this.commentLines);
  return ret;
};

LDRPartDescription.prototype.toLDRColored = function (loader, c) {
  let pt = loader.getPartType(this.ID);
...
const LDRStepRotation = function (x, y, z, type) {
  this.x = parseFloat(x);
  this.y = parseFloat(y);
  this.z = parseFloat(z);
...
LDRStepRotation.equals = function (a, b) {
  let aNull = !a;
  let bNull = !b;
  if (aNull && bNull) {
    return true;
...
LDRStepRotation.prototype.isDefault = function () {
  return this.type === 'REL' && this.x === 0 && this.y === 0 && this.z === 0;
};

LDRStepRotation.prototype.clone = function () {
  return new LDRStepRotation(this.x, this.y, this.z, this.type);
};

LDRStepRotation.prototype.toLDR = function () {
  return '0 ROTSTEP ' + this.x + ' ' + this.y + ' ' + this.z + ' ' + this.type + '\r\n';
};

// Get the rotation matrix by looking at the default camera position:
LDRStepRotation.getAbsRotationMatrix = function () {
  let looker = new Object3D();
  looker.position.set(-10000, -7000, -10000);
  looker.lookAt(new Vector3());
  looker.updateMatrix();
  let m = new Matrix4();
  m.extractRotation(looker.matrix);
  return m;
};
LDRStepRotation.ABS = LDRStepRotation.getAbsRotationMatrix();

/* 
   Specification: https://www.lm-software.com/mlcad/Specification_V2.0.pdf (page 7 and 8)
*/
LDRStepRotation.prototype.getRotationMatrix = function (defaultMatrix) {
  ...
  if (this.type === 'REL') {
    ...
    // this.type === ABS
    ret.multiplyMatrices(LDRStepRotation.ABS, rotationMatrix);
  }
  return ret;
};

let LDRStepIdx = 0;
const LDRStep = function () {
  this.idx = LDRStepIdx++;
  ...
  this.cnt = -1;
  this.original = undefined; // LDRStep
};

LDRStep.prototype.hasPrimitives = function () {
  return (
    this.lines.length > 0 ||
...
LDRStep.prototype.pack = function (obj, saveCommentLines) {
  let arrayI = [],
    arrayF = [],
    arrayS = [];
...
LDRStep.prototype.packInto = function (arrayI, arrayF, arrayS, subModelMap, saveCommentLines) {
  ...
  function handleSubModel(sm) {
    ...
    arrayI.push(subModelMap[sm.ID] * 4 + (sm.invertCCW ? 2 : 0) + (sm.cull ? 1 : 0)); // Encode these three properties into a single int.
    if (saveCommentLines) {
...
LDRStep.prototype.unpack = function (obj) {
  let arrayI = obj.ai;
  let arrayF = obj.af;
...
LDRStep.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  arrayS,
  idxI,
  ...
  for (let i = 0; i < numSubModels; i++) {
    ...
    let position = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
    let rotation = new Matrix3();
    rotation.set(
      arrayF[idxF++],
      arrayF[idxF++],
...
LDRStep.prototype.getTexmapPlacements = function (seen) {
  let ret = 0;
  function handle(p) {
    if (!p.tmp) {
      return;
...
LDRStep.prototype.cloneColored = function (colorID) {
...
LDRStep.prototype.toLDR = function (loader, prevStepRotation, isLastStep) {
  let ret = '';
  ...
  if (!this.rotation) {
    ...
    if (LDRStepRotation.equals(this.rotation, prevStepRotation)) {
      ret += '0 STEP\r\n';
    } else {
...
LDRStep.prototype.addLine = function (c, p1, p2, texmapPlacement = null) {
  this.lines.push(new LDR.Line2(c, p1, p2, texmapPlacement));
  texmapPlacement && texmapPlacement.use();
};

LDRStep.prototype.addTriangle = function (
  c,
  p1,
  p2,
  p3,
...
LDRStep.prototype.addQuad = function (
  c,
  p1,
  p2,
  p3,
...
LDRStep.prototype.addConditionalLine = function (c, p1, p2, p3, p4, texmapPlacement = null) {
 ...
 */
LDRStep.prototype.containsNonPartSubModels = function (loader) {
...
LDRStep.prototype.containsPartSubModels = function (loader) {
  if (this.subModels.length === 0) {
    return false;
  }
  let firstSubModel = loader.getPartType(this.subModels[0].ID);
...
LDRStep.prototype.countParts = function (loader) {
  if (this.cnt >= 0) {
    return this.cnt;
  }
  let cnt = 0;
...
LDRStep.prototype.removePrimitivesAndSubParts = function (loader) {
...
LDRStep.prototype.generateThreePart = function (
  loader,
  colorID,
  position,
  rotation,
  ...
  for (let i = 0; i < this.subModels.length; i++) {
    ...
    if (pt.replacement) {
      ...
      if (replacementSubModel) {
        ...
        loader.onError({
          message: 'Unloaded replaced sub model: ' + pt.replacement + ' replacing ' + sm.ID,
          subModel: pt.ID,
        });
      }
    }
...
LDRStep.prototype.toLDRColored = function (loader, prevStepRotation, isLastStep, c) {
  let ret = '';
  ...
  if (!this.r) {
    ...
    // We have a rotation. Check against prev:
    if (LDRStepRotation.equals(this.r, prevStepRotation)) {
      ret += '0 STEP\r\n';
    } else {
      ret += this.r.toLDR();
    }
...
LDRStep.prototype.al = function (x) {
...
LDRStep.prototype.at = function (t) {
...
LDRStep.prototype.aq = function (q) {
...
LDRStep.prototype.asm = function (p = null, r = null, id = '', c = 16, cull = true, ccw = false) {
  if (p === null) {
  ...
  this.addSubModel(new LDRPartDescription(c, p, r, id + '.dat', cull, ccw));
};

LDRStep.prototype.acl = function (lines) {
  for (let i = 0; i < lines.length; i += 12) {
    this.addConditionalLine(
      24,
      new Vector3(lines[i], lines[i + 1], lines[i + 2]),
      new Vector3(lines[i + 3], lines[i + 4], lines[i + 5]),
      new Vector3(lines[i + 6], lines[i + 7], lines[i + 8]),
      new Vector3(lines[i + 9], lines[i + 10], lines[i + 11]),
    );
  }
};
...
LDR.Line2.prototype.unpackFrom = function (arrayI, arrayF, idxI, idxF, texmapPlacementMap) {
  ...
  this.p1 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
  this.p2 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
...
LDR.Line3.prototype.unpackFrom = function (arrayI, arrayF, idxI, idxF, texmapPlacementMap) {
  ...
  this.p1 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
  this.p2 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
  this.p3 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
...
LDR.Line3.prototype.toLDR = function () {
  return (
    '3 ' + this.c + ' ' + this.p1.toLDR() + ' ' + this.p2.toLDR() + ' ' + this.p3.toLDR() + '\r\n'
  );
};
...
LDR.Line4.prototype.unpackFrom = function (arrayI, arrayF, idxI, idxF, texmapPlacementMap) {
  ...
  this.p2 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
  this.p3 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
  this.p4 = new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]);
...
const LDRPartType = function () {
  this.name = undefined; // The value for '0 FILE' and '0 Name:'.
...
LDRPartType.prototype.toStudioFile = function (ldrLoader) {
  if (!this.isPart) {
    throw (
      'The part type ' +
      this.ID +
      ' cannot be converted to a Studio 2.0 file since it is not a part'
    );
  }
  ...
  ret +=
    '0 ' +
    (this.modelDescription ? this.modelDescription : '') +
    '\r\n0 Name: ' +
    this.ID +
    '\r\n0 Author: ' +
 ...
 */
LDRPartType.prototype.setReferencedFrom = function (ldrLoader) {
...
LDRPartType.prototype.canBePacked = function () {
  return (
...
LDRPartType.prototype.encodeHeader = function () {
  return (this.CCW ? 2 : 0) + (this.certifiedBFC ? 1 : 0);
};

LDRPartType.prototype.pack = function (loader) {
  let ret = {};
  let id = this.ID;
  if (id.endsWith('.dat')) {
...
LDRPartType.prototype.decodeHeader = function (encoded) {
  this.certifiedBFC = encoded % 2 === 1;
  this.CCW = Math.floor(encoded / 2) % 2 === 1;
};

LDRPartType.prototype.unpack = function (obj) {
...
LDRLoader.prototype.purgePart = function (ID) {
  let self = this;
  let purged = {};
  let toPurge = [ID];
 ...
 */
LDRPartType.prototype.purgePart = function (ID) {
  if (this.isPart) {
...
LDRPartType.prototype.toLDR = function (loader, skipFile) {
  let ret = '';
  if (!skipFile) {
    ret = '0 FILE ' + this.ID + '\r\n';
  }
  ...
  if (this.steps.length > 0) {
    ...
    this.steps.forEach(
      (step, idx, a) =>
        (ret += step.toLDR(loader, idx === 0 ? null : a[idx - 1].rotation, idx === a.length - 1)),
    );
  }
  ret += '\r\n';
  return ret;
...
LDRPartType.prototype.toLDRColored = function (loader, c) {
  ...
  if (this.steps.length > 0) {
    ...
    this.steps.forEach(
      (step, idx, a) =>
        (ret += step.toLDRColored(
          loader,
          idx === 0 ? null : a[idx - 1].r,
          idx === a.length - 1,
          c,
        )),
    );
...
LDRPartType.prototype.removePrimitivesAndSubParts = function (loader, parentID) {
...
LDRPartType.prototype.generateThreePart = function (loader, c, p, r, cull, inv, mc, pd, taskList) {
  ...
  for (let tc in this.geometry.triangleGeometries) {
    ...
    let mesh = new Mesh(g.clone(), material); // Using clone to ensure matrix in next line doesn't affect other usages of the geometry.
  ...
  for (let tc in this.geometry.lineGeometries) {
    ...
    let normalLines = new LineSegments(this.geometry.lineGeometries[tc], material);
  ...
  for (let tc in this.geometry.conditionalLineGeometries) {
    ...
    let conditionalLines = new LineSegments(this.geometry.conditionalLineGeometries[tc], material);
...
LDRPartType.prototype.isReplacedPart = function () {
...
LDRPartType.prototype.countParts = function (loader) {
...
LDR.TexmapPlacement = function () {
  ...
  this.type = undefined; // 0 = planar, 1 = cylindrical, 2 = spherical.
  this.p = [];
  ...
  this.idx = undefined; // Index in LDR.TexmapPlacements
};
...
LDR.TexmapPlacement.prototype.clone = function () {
  ...
  x.type = this.type;
  x.p = this.p.map((v) => new Vector3(v.x, v.y, v.z));
  x.a = this.a;
  x.b = this.b;
...
LDR.TexmapPlacement.prototype.setFromParts = function (parts) {
  ...
  for (let i = 0; i < 3; i++) {
    this.p.push(
      new Vector3(parseFloat(parts[idx++]), parseFloat(parts[idx++]), parseFloat(parts[idx++])),
    );
  }
...
LDR.TexmapPlacement.prototype.unpackFrom = function (
  ...
  for (let i = 0; i < 3; i++) {
    this.p.push(new Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]));
  }
...
LDR.attachGlowPassesForObjects = function (map, w, h, scene, camera, composer) {
  ...
  for (let color in map) {
    ...
    let glowPass = new OutlinePass(new Vector2(w, h), scene, camera, map[color]);
    let lum = LDR.Colors.getLuminance(color);
...
LDR.MeshCollector.prototype.overwriteColor = function (color) {
  ...
  function handle(obj) {
    ...
    let color4 =
      edge && LDR.Options && LDR.Options.lineContrast === 0
...
LDRLoader.prototype.toLDRStudio = function (c) {
  let self = this;
  ...
  function findColorsFor(id, c) {
    ...
    sc[c] = true;
    pt.steps.forEach((step) =>
      step.subModels.forEach((sm) => findColorsFor(sm.ID, sm.c === 16 ? c : sm.c)),
    );
  }
...
export { LDRPartDescription, LDRPartType, LDRStep, LDRLoader };
