...
class LDrawParsedCache {
  ...
  async fetchData(fileName, baseFileName) {
    ...
    // Check if we have a fileMap with the exact path
    const loader = this.loader;

    // Try to look up in fileMap
    ...
    if (!foundInMap && !fileName.includes('/')) {
      // Not found in map, fall back to heuristics
      if (!baseFileName) {
        subobjectURL = 'parts/' + subobjectURL;
      } else {
        // if the first and second character are numbers
        if (fileName.substr(0, 4).match(/^\d\d\d\d$/)) {
          subobjectURL = 'parts/' + subobjectURL;
        } else {
  ...
  async ensureDataLoaded(fileName, baseFileName) {
    const key = fileName.toLowerCase();
    if (!(key in this._cache)) {
      // replace the promise with a copy of the parsed data for immediate processing
      this._cache[key] = this.fetchData(fileName, baseFileName).then((text) => {
...
class LDrawPartsGeometryCache {
  ...
  async processIntoMesh(info) {
    ...
    if (info.faces.length > 0) {
      group.add(
        createObject(this.loader, info.faces, 3, false, info.totalFaces),
      );
    }
  ...
  async loadModel(fileName, baseFileName) {
    const parseCache = this.parseCache;
    const key = fileName.toLowerCase();
    if (this.hasCachedModel(fileName)) {
      ...
      await parseCache.ensureDataLoaded(fileName, baseFileName);
      ...
      if (material !== null) {
        if (elementSize === 3) {
          ...
          if (isConditionalSegments) {
            ...
            materials.push(
              loader.conditionalEdgeMaterialCache.get(edgeMaterial),
            );
          } else {
...
class LDrawLoader extends Loader {
  ...
  setConditionalLineMaterial(type) {
    ...
    this.missingConditionalEdgeColorMaterial = new this.ConditionalLineMaterial(
      { name: Loader.DEFAULT_MATERIAL_NAME, fog: true, color: 0xff00ff },
    );
    return this;
  }
  ...
  parseColorMetaDirective(lineParser) {
    ...
    if (luminance !== 0) {
      material.emissive
        .setStyle(fillColor, COLOR_SPACE_LDRAW)
